...
export default function ACreateEvent() {
  ...
  const fetchlocationlatlong = async () => {
    const res = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=json&lat=${ismaplatitude}&lon=${ismaplongitude}`,
    );
    console.log("Api result for location,latitude and longitude is : ", res);
  };
  useEffect(() => {
  ...
  console.log(ismaplatitude, ismaplongitude);
...
export default function ACreateEvent() {
  ...
  const { data, loading } = useQuery<GET_Event_Detail_Interface>(
    GET_EVENT_QUERY,
    {
      ...
      // skip query when no editId to avoid fetching with null
      skip: !editId,
    },
  );
  ...
  //  fetch human-readable location from lat/lng using Nominatim
  ...
  // update lat/lng in formInput when map coords change
  useEffect(() => {
    if (ismaplatitude && ismaplongitude) {
      setFormInput((prev) => ({
        ...prev,
  ...
  // prefill form when editing — use toDatetimeLocal for date fields
  useEffect(() => {
    if (res) {
      setFormInput({
        title: res.title ?? "",
  ...
  const handleFormSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // use existing imagePreview (from edit) if no new file chosen
        ...
        <div className={styles.card}>
          ...
          <form onSubmit={handleFormSubmit}>
            ...
            <div className={styles.mapSection}>
              ...
              {/* Map component — clicking sets ismaplatitude/ismaplongitude in context */}
              <div
                style={{
                  marginBottom: 15,
                  borderRadius: 8,
